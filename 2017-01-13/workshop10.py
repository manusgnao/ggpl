...
def createEst(l,i,h,s1):
  if l <= len(levelEst)-1:
    if i < len(levelEst[l]):
      ...
      if l==0:
        a_texture = TEXTURE([wallStone, TRUE, FALSE, 1, 1, 0, 10, 1])(a_off)
      else:
        if l==3:
          a_texture = TEXTURE([wallStone, TRUE, FALSE, 1, 1, 0, 10, 1])(a_off)
        else:
          if l==2:
            a_texture = TEXTURE([wallTexture, TRUE, FALSE, 1, 1, 0, 12, 1])(a_off)
          if l==5:
            a_texture = TEXTURE([wallTexture, TRUE, FALSE, 1, 1, 0, 12, 1])(a_off)
          else:
...
def buildStair(x,y,z,tempWeight,tempHeight,tempStruct,dx,dy):
  if(2.0*length/5.0<tempWeight<4.0*length/5.0):
    step=COLOR(BROWN)(CUBOID([x, y*3, z]))
    traslation=STRUCT([T([1,2,3])([wall/3+dx,tempWeight+dy,tempHeight]),step])
  ...
  if tempWeight < length+y:
    buildStair(x, y, z, tempWeight, tempHeight, stair,dx,dy)
  else:
...
def Stairs(l,i,h,s1):
  if l <= len(levelStairs)-1:
    if i < len(levelStairs[l]):
      coords = levelStairs[l][i]
      arrayCoords = coords.split(",")
      elem = np.array(arrayCoords, dtype=float)
      build = MKPOL([[[elem[0],elem[1],0.0],[elem[2],elem[3],0.0]],[[1,2]],[1]])
      buildOffset = OFFSET([1.5, 1.5, 1.5])(build)
      buildStair(70,10,10,0,0,buildOffset,elem[2],elem[3])
...
def buildDoors(l,i,h,s1):
  if l <= len(levelPorte)-1:
    if i < len(levelPorte[l]):
      ...
      buildTexture = TEXTURE([doorTexture, TRUE, FALSE, 1, 1, 0, 2, 1])(buildOffset)
...
def build(elem,h):
  ...
  q1 = TEXTURE([metalTexture, TRUE, FALSE, 1, 1, 0, 1, 1])(q1)
...
def createHouse():
  ...
  stair = Stairs(0,0,0.0,initStruct)
  ...
  #house=STRUCT([house,T(3)(3.0),stair])
  #VIEW(house)
  VIEW(stair)
createHouse()